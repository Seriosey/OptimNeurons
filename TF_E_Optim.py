...
class VonMissesSpatialModulation(VonMissesGenerator):
    def __init__(self, params, dt=0.1):
        ...
        sigma_sp = tf.Variable(tf.zeros(len(params), dtype=tf.float32))
        maxFiring = tf.Variable(tf.zeros_like(sigma_sp))
        v_an = tf.Variable(tf.zeros_like(sigma_sp))
        mean_spike_rates = tf.Variable(tf.zeros_like(sigma_sp))
        sp_centers = tf.Variable(tf.zeros_like(sigma_sp))
...
def simulate(X0 : tf.Variable, Duration, dt, Cm, animal_velocity, params_generators, params_synapses):
    ...
    tau_d = tf.cast(params_synapses_["tau_d"], dtype=tf.float32)
    tau_r = tf.cast(params_synapses_["tau_r"], dtype=tf.float32)
    tau_f = tf.cast(params_synapses_["tau_f"], dtype=tf.float32)
    Uinc = tf.cast(params_synapses_["Uinc"], dtype=tf.float32)
    pconn = tf.cast(params_synapses_["pconn"], dtype=tf.float32)
    ...
    t = tf.linspace(0, Duration, int(Duration/dt))
    ...
    for ts in t:
        ...
        #g_hist = tf.concat([g_hist, [R_plus]], axis=0)
...
def Loss(X : tf.Variable, Duration, dt, Cm, animal_velocity, params_generators, params_synapses, target_params):
    ...
    tc = 0.5*Duration
    t = tf.linspace(0, Duration, Duration/dt)
